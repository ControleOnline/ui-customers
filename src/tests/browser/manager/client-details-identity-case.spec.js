/**
 * Browser smoke for app-community#626.
 * fluxo: outros
 * Cadastro/detalhe de colaborador: name/alias keep the capitalization typed
 * by the user (no toUpperCase on input or persist).
 */
...
const attachSession = async (page, appVersionValue) => {
...
const mockIdentityApi = async page => {
  ...
  await page.route(`${API_ORIGIN}/**`, async route => {
    ...
    if (pathname === 'people' && method === 'POST') {
      const body = await request.postDataJSON().catch(() => ({}));
      peopleCreates.push({pathname, body});
      return route.fulfill({
        status: 201,
        headers: jsonHeaders(),
        body: JSON.stringify({
    ...
    if (pathname === 'people/company' || pathname === 'people/company/') {
  ...
  await attachSession(page, appVersion);
  return {peopleWrites, peopleCreates};
};

test.describe('client-details identity case preservation (#626)', () => {
  test('GeneralTab keeps exact mixed case on name/alias in people PUT body', async ({
    page,
  }) => {
    ...
    const MIXED_NAME = 'Maria Silva';
    const MIXED_ALIAS = 'Mari';
  ...
  test('EmployeesTab create modal posts mixed-case name/alias', async ({page}) => {
    const api = await mockIdentityApi(page);

    await page.goto('/client-details?clientId=40&initialTab=contacts');

    await expect(page.getByText('Dados Cadastrais', {exact: true})).toBeVisible({
      timeout: 15000,
    });

    const contactsTab = page.getByText(/Contatos|Contacts/i).first();
    if (await contactsTab.count()) {
      await contactsTab.click();
    }

    const addButton = page.getByTestId('employee-add-button').first();
    ...
    const aliasInput = page.getByTestId('employee-alias-input');
    await expect(nameInput).toBeVisible({timeout: 10000});

    await nameInput.fill(MIXED_NAME);
    await aliasInput.fill(MIXED_ALIAS);
    await page.getByText(/^Salvar$/i).last().click();

    await expect.poll(() => api.peopleCreates.length, {timeout: 15000}).toBeGreaterThanOrEqual(1);
    const body = api.peopleCreates[0].body || {};
    expect(String(body.name)).toBe(MIXED_NAME);
    expect(String(body.alias)).toBe(MIXED_ALIAS);
  });
});
